...
func LoadOrderAssembledConsumerConfig() (*OrderAssembledConsumerConfig, error) {
	topic := os.Getenv("ORDER_ORDER_ASSEMBLED_TOPIC_NAME")
	if topic == "" {
		topic = "order.assembled"
	}

	consumerGroup := os.Getenv("ORDER_ORDER_ASSEMBLED_CONSUMER_GROUP_ID")
	if consumerGroup == "" {
		consumerGroup = "order-group-order-assembled"
	}
