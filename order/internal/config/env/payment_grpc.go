...
func LoadPaymentGRPCConfig() (*PaymentGRPCConfig, error) {
	...
	portStr := os.Getenv("PAYMENT_GRPC_PORT")
	if portStr == "" {
		portStr = "50052"
	}
	port, err := strconv.Atoi(portStr)
	if err != nil {
		return nil, fmt.Errorf("invalid PAYMENT_GRPC_PORT: %w", err)
	}
...
func (c *PaymentGRPCConfig) Address() string { return fmt.Sprintf("%s:%d", c.host, c.port) }
